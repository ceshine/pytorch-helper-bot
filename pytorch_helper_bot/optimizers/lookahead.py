...
class LookaheadCallback(Callback):
    def on_eval_starts(self, bot: BaseBot):
        if hasattr(bot.optimizer, "_backup_and_load_cache"):
            print("load slow parameters")
            bot.optimizer._backup_and_load_cache()

    def on_eval_ends(self, bot: BaseBot, metrics: Dict[str, Tuple[float, str]]):
        if hasattr(bot.optimizer, "_clear_and_load_backup"):
            print("load fast parameters")
            bot.optimizer._clear_and_load_backup()
